"""
Dense Matrix Multiplication GPU Benchmark - Database I/O Workflow

Benchmarks dense matrix multiplication on GPU using PyTorch with database I/O:
...
def run_gpu_benchmark(input_a, input_b, num_runs=1):
    """Run GPU dense multiplication benchmark with database I/O workflow."""
    
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: Dense GPU Multiplication Benchmark")
    print(f"{'='*70}")
    ...
    out_name = f"multiplication_dense_gpu_{result_dense.shape[0]}x{result_dense.shape[1]}_result.csv"
    ...
    metrics_path = os.path.join(results_dir, "metrics_multiplication_dense_gpu.json")
...
def main():
    parser = argparse.ArgumentParser(description='Dense GPU multiplication benchmark')
