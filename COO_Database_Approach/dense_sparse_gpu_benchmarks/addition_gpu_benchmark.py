"""
COO GPU Addition Benchmark - Database I/O Workflow

Benchmarks sparse matrix addition on GPU using PyTorch with database I/O:
...
def run_gpu_benchmark(input_a, input_b, num_runs=1):
    """Run GPU addition benchmark with database I/O workflow."""
    
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: COO GPU Addition Benchmark")
    print(f"{'='*70}")
    ...
    shape = (max(shape_A[0], shape_B[0]), max(shape_A[1], shape_B[1]))
    ...
    for _ in range(num_runs):
        ...
        sparse_A = torch.sparse_coo_tensor(indices_A, values_A, shape, device=device).coalesce()
        
        indices_B = torch.from_numpy(np.vstack([rows_B, cols_B])).to(device)
        values_B = torch.from_numpy(vals_B).to(device)
        sparse_B = torch.sparse_coo_tensor(indices_B, values_B, shape, device=device).coalesce()
        
        result_sparse = (sparse_A + sparse_B).coalesce()
    ...
    print(f"  Result: {nnz_result:,} non-zeros")
    ...
    out_name = f"addition_gpu_coo_{shape[0]}x{shape[1]}_{nnz_A}A_{nnz_B}B.csv"
    ...
    metrics = {
        ...
        "result": {"shape": list(shape), "nnz": nnz_result, "file": out_path},
        "timing": {
    ...
    metrics_path = os.path.join(results_dir, "metrics_addition_gpu.json")
...
def main():
    parser = argparse.ArgumentParser(description='COO GPU addition benchmark')
