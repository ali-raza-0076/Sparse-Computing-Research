"""
Dense Matrix Multiplication Benchmark (Single-Threaded, Database I/O Workflow)

Database I/O Workflow:
- Phase 1: Read from secondary storage (CSV files in COO format)
- Phase 2: Process in RAM (Dense multiplication using NumPy)
...
- Parallelism: Single-threaded (NumPy default)
- Metrics: I/O time breakdown, compute time, throughput, overhead percentages

Output:
- results/multiplication_dense_<dims>_result.csv (result matrix in COO format)
- results/metrics_multiplication_dense.json (detailed performance metrics)
"""
import numpy as np
import time
import csv
...
def dense_multiplication(A, B, num_runs=3):
    """
    Benchmark dense matrix multiplication using NumPy.
...
def run_dense_benchmark(input_a, input_b, num_runs=1):
    """
    Run dense multiplication benchmark with database I/O workflow.
    ...
    """
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: Dense Multiplication Benchmark (Single-Threaded)")
    print(f"{'='*70}")
    ...
    print(f"\n[PHASE 2] Processing in RAM (Dense NumPy multiplication)...")
    compute_start = time.perf_counter()
    avg_time, std_time, result = dense_multiplication(A_dense, B_dense, num_runs=num_runs)
    ...
    out_name = f"multiplication_dense_{result.shape[0]}x{result.shape[1]}_result.csv"
    ...
    metrics = {
        ...
        "execution_mode": "single_threaded_dense",
        "matrix_a": {
            "file": input_a,
            "shape": list(shape_A),
            "nnz": nnz_A,
    ...
    metrics_path = os.path.join(results_dir, "metrics_multiplication_dense.json")
...
def main():
    parser = argparse.ArgumentParser(description='Dense matrix multiplication: multiply two input CSVs (COO format) using dense NumPy arrays')
    ...
    run_dense_benchmark(args.input_a, args.input_b, num_runs=args.num_runs)
