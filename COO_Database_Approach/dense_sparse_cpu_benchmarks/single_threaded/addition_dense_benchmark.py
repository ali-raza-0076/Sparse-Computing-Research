"""
Dense Matrix Addition Benchmark (Single-Threaded, Database I/O Workflow)

Database I/O Workflow:
- Phase 1: Read from secondary storage (CSV files in COO format)
- Phase 2: Process in RAM (Dense addition using NumPy)
...
Output:
- results/addition_dense_<dims>_result.csv (result matrix in COO format)
- results/metrics_addition_dense.json (detailed performance metrics)
"""
import numpy as np
import time
import csv
...
def dense_addition(A, B, num_runs=3):
    """
    Benchmark dense matrix addition using NumPy.
...
def run_dense_benchmark(input_a, input_b, num_runs=1):
    """
    Run dense addition benchmark with database I/O workflow.
    ...
    """
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: Dense Addition Benchmark (Single-Threaded)")
    print(f"{'='*70}")
    ...
    if A_dense.shape != B_dense.shape:
        ...
        A_padded = np.zeros((max_rows, max_cols), dtype=np.float64)
        B_padded = np.zeros((max_rows, max_cols), dtype=np.float64)
    ...
    print(f"\n[PHASE 2] Processing in RAM (Dense NumPy addition)...")
    compute_start = time.perf_counter()
    avg_time, std_time, result = dense_addition(A_dense, B_dense, num_runs=num_runs)
    ...
    out_name = f"addition_dense_{result.shape[0]}x{result.shape[1]}_result.csv"
    ...
    metrics_path = os.path.join(results_dir, "metrics_addition_dense.json")
...
def main():
    parser = argparse.ArgumentParser(description='Dense matrix addition: add two input CSVs (COO format) using dense NumPy arrays')
