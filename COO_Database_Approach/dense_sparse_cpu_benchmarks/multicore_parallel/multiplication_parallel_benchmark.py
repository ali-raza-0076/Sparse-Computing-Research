"""
COO Parallel Multiplication Benchmark (Multicore, Database I/O Workflow)

Database I/O Workflow:
- Phase 1: Read from secondary storage (CSV files)
- Phase 2: Process in RAM (COO parallel multiplication with multiple cores)
- Phase 3: Write to secondary storage (CSV result)

Features:
- Format: Pure COO (coordinate list: i,j,v triplets)
- Parallelism: Multicore via Numba (uses CPU count by default)
- Metrics: I/O time breakdown, compute time, throughput, overhead percentages

Output:
- results/multiplication_parallel_coo_B×A_<dims>_<cores>cores_result.csv (result matrix)
- results/metrics_multiplication_parallel.json (detailed performance metrics)
...
def multiply_matrices_parallel(coo_A, coo_B, num_workers=None):
    """
    Parallel multiplication of COO matrices.
    
    Note: The COO implementation uses Numba JIT which provides parallelism.
    This wrapper demonstrates the multicore capability.
...
def run_parallel_benchmark(input_a, input_b, num_workers=None, num_runs=1):
    """
    Run parallel multiplication benchmark with database I/O workflow.
    ...
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: COO Parallel Multiplication Benchmark ({num_workers} cores)")
    print(f"{'='*70}")
    ...
    if coo_B.shape[1] == coo_A.shape[0]:
        ...
        raise ValueError(f"Incompatible dimensions: B is {coo_B.shape}, A is {coo_A.shape}")
    
    print(f"\n[PHASE 2] Processing in RAM with {num_workers} cores (Numba parallelism)...")
    compute_start = time.perf_counter()
    times = []
    for _ in range(num_runs):
        run_start = time.perf_counter()
        result = multiply_matrices_parallel(first_matrix, second_matrix, num_workers=num_workers)
    ...
    out_name = f"multiplication_parallel_coo_{op_label}_{result.shape[0]}x{result.shape[1]}_{num_workers}cores_result.csv"
    ...
    metrics_path = os.path.join(results_dir, "metrics_multiplication_parallel.json")
...
def main():
    parser = argparse.ArgumentParser(description='COO parallel multiplication: multiply two input CSVs using multiple cores')
