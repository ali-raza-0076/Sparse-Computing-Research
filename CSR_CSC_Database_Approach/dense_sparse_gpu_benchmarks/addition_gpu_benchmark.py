"""
GPU Addition Benchmark - CSR Format with Database I/O

Benchmarks sparse matrix addition using PyTorch CSR on GPU with database I/O:
- Phase 1: Read from disk (CSV)
- Phase 2: Convert to CSR, transfer to GPU, add, transfer back
...
def coo_to_csr_torch(rows, cols, vals, shape, device='cuda'):
    """Convert COO to PyTorch CSR sparse tensor."""
    indices = torch.tensor([rows, cols], dtype=torch.long)
    values = torch.tensor(vals, dtype=torch.float32)
    coo_tensor = torch.sparse_coo_tensor(indices, values, shape).coalesce()
    csr_tensor = coo_tensor.to_sparse_csr().to(device)
    return csr_tensor


def csr_to_coo_lists(csr_tensor):
    """Convert PyTorch CSR back to COO lists for CSV writing."""
    coo_tensor = csr_tensor.to_sparse_coo().cpu()
...
def run_gpu_addition_benchmark(input_a, input_b, num_runs=1):
    """Run GPU CSR addition benchmark with database I/O workflow."""
    print(f"\n{'='*70}")
    print(f"GPU DATABASE I/O WORKFLOW: CSR Addition Benchmark")
    print(f"{'='*70}")
    ...
    print(f"\n[PHASE 2] Processing on {device.upper()} (CSR format)...")
    compute_start = time.perf_counter()
    
    print(f"  Converting COO -> CSR and transferring to {device.upper()}...")
    csr_A = coo_to_csr_torch(rows_A, cols_A, vals_A, shape_A, device)
    csr_B = coo_to_csr_torch(rows_B, cols_B, vals_B, shape_B, device)
    ...
    for _ in range(num_runs):
        ...
        result_csr = csr_A + csr_B
        
        torch.cuda.synchronize()
        run_end = time.perf_counter()
        times.append(run_end - run_start)
    
    result_csr = result_csr.cpu()
    ...
    rows_out, cols_out, vals_out = csr_to_coo_lists(result_csr)
    ...
    output_file = f'results/addition_gpu_csr_{result_csr.shape[0]}x{result_csr.shape[1]}.csv'
    ...
    metrics = {
        ...
        'result_shape': list(result_csr.shape)
    }
    
    metrics_file = 'results/metrics_addition_gpu_csr.json'
...
def main():
    parser = argparse.ArgumentParser(description='GPU CSR Addition Benchmark - Database I/O')
    ...
    run_gpu_addition_benchmark(args.input_a, args.input_b, args.num_runs)
