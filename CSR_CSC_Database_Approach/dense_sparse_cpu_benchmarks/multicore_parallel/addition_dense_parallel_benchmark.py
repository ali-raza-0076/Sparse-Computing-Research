"""
Dense Matrix Addition Multicore Parallel CPU Benchmark - Database I/O Workflow

Benchmarks dense matrix addition on multicore CPU using NumPy with database I/O:
- Phase 1: Read from disk (CSV in COO format)
- Phase 2: Convert to dense and compute (NumPy automatically uses multiple cores)
- Phase 3: Write to disk (COO format)
"""
import numpy as np
import time
import csv
...
def run_parallel_benchmark(input_a, input_b, num_runs=1):
    """Run multicore parallel dense addition benchmark with database I/O workflow."""
    
    print(f"\n{'='*70}")
    print(f"DATABASE I/O WORKFLOW: Dense Parallel Addition Benchmark")
    print(f"{'='*70}")
    print(f"  Using NumPy for multicore parallel execution")
    ...
    print(f"\n[PHASE 2] Processing with multicore parallel...")
    ...
    out_name = f"addition_dense_parallel_{result_dense.shape[0]}x{result_dense.shape[1]}_result.csv"
...
def main():
    parser = argparse.ArgumentParser(description='Dense parallel addition benchmark')
    ...
    run_parallel_benchmark(args.input_a, args.input_b, num_runs=args.num_runs)
