"""
GPU Matrix Addition Benchmark: CSR + CSR Sparse vs Dense
Mirrors CPU benchmark format with command-line arguments for matrix size and sparsity.
Compares sparse GPU (PyTorch) vs dense GPU addition performance.
"""
...
def generate_sparse_matrices(size, sparsity_percent, seed_a=42, seed_b=123):
    """
    Generate two sparse matrices (both CSR) for addition.
    ...
    Returns:
        ...
        B_csr: Second matrix in CSR format
    ...
    return A_csr, B_csr, actual_nnz, actual_sparsity
...
def benchmark_gpu_sparse(A_sparse_gpu, B_sparse_gpu, runs=3):
    """
    Benchmark GPU sparse matrix addition (CSR + CSR).
...
def benchmark_gpu_dense(A_gpu, B_gpu, runs=3):
    """
    Benchmark GPU dense matrix addition.
...
def main():
    parser = argparse.ArgumentParser(description='GPU Matrix Addition Benchmark: CSR + CSR')
    ...
    print("\n" + "=" * 80)
    print("GPU MATRIX ADDITION BENCHMARK: CSR + CSR")
    ...
    A_csr, B_csr, actual_nnz, actual_sparsity = generate_sparse_matrices(
    ...
    B_gpu_sparse = csr_to_pytorch_sparse(B_csr, device)
    
    print(f"\nBenchmarking GPU Sparse (CSR + CSR)...")
    ...
    base_filename = f"addition_gpu_{args.size}x{args.size}_{sparsity_str}pct"
    ...
    with open(txt_file, "w") as f:
        f.write("=" * 80 + "\n")
        f.write("GPU MATRIX ADDITION BENCHMARK: CSR + CSR\n")
        f.write("=" * 80 + "\n\n")
