"""
Benchmark script for TRUE multiprocessing CSR sparse matrix addition vs dense NumPy addition.
...
def benchmark_addition(size, sparsity_percent=99.9, num_cores=16):
    """
    Benchmark sparse CSR addition (multiprocess) vs dense NumPy addition.
    ...
    print(f"\n--- Sparse CSR Addition (TRUE Multiprocessing, {num_cores} cores) ---")
    
    _ = sparse_add_csr_multiprocess(A_indptr, A_indices, A_data, 
                                    B_indptr, B_indices, B_data, num_cores=num_cores)
    
    start = time.perf_counter()
    C_indptr, C_indices, C_data = sparse_add_csr_multiprocess(
    ...
    filename = f"addition_multiprocess_csr_{size}x{size}_{sparsity_percent}pct_{timestamp}.txt"
    filepath = os.path.join(results_dir, filename)
    
    with open(filepath, 'w') as f:
        f.write(f"TRUE Multiprocessing CSR Addition Benchmark\n")
        f.write(f"{'='*80}\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Benchmark TRUE multiprocess CSR addition')
    ...
    benchmark_addition(args.size, args.sparsity, args.cores)
