...
def benchmark_sparse_parallel(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, size, num_cores, num_runs=3):
    """
    Benchmark sparse COO addition with multicore parallelism using file-based API.
    ...
    for _ in tqdm(range(num_runs), desc=f"  Sparse COO ({num_cores}-core parallel)", leave=False):
        ...
        try:
            ...
            add_matrices_parallel_csr(temp_a, temp_b, temp_out, shape=(size, size), num_workers=num_cores)
...
def main():
    ...
    base_name = f"addition_parallel_{matrix_size}x{matrix_size}_results"
