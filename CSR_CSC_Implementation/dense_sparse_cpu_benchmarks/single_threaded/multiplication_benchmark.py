...
- Sparse CPU (CSR×CSC single-threaded algorithm)
...
def benchmark_sparse_multiplication(coo_A, coo_B, num_runs=3):
    """
    Benchmark sparse CSR×CSC multiplication (single-threaded).
    ...
    """
    _ = sparse_multiply_from_coo(coo_A, coo_B)
    
    times = []
    for _ in tqdm(range(num_runs), desc="  Sparse CSR×CSC (single-threaded)", leave=False):
        start = time.perf_counter()
        result = sparse_multiply_from_coo(coo_A, coo_B)
...
def run_comparison(size, sparsity_percent, num_runs=3):
    ...
    print(f"\nBenchmarking SPARSE (CSR×CSC, single-threaded)...")
    ...
    return {
        ...
        "algorithm": "CSR×CSC (single-threaded)",
        "num_runs": num_runs
    }
...
def main():
    ...
    print(f"Algorithm: CSR×CSC (single-threaded)")
    ...
    with open(txt_path, "w", encoding='utf-8') as f:
        ...
        f.write("="*70 + "\n\n")
        f.write(f"Algorithm: CSR×CSC (single-threaded)\n")
    ...
    print(f"  - multiplication_{matrix_size}x{matrix_size}_results.json")
    print(f"  - multiplication_{matrix_size}x{matrix_size}_results.txt")
    print(f"  - multiplication_{matrix_size}x{matrix_size}_results.csv")
