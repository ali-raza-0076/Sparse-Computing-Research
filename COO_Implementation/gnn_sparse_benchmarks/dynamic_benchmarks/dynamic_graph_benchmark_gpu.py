"""
GPU Dynamic Graph Benchmark: Incremental Edge Addition (COO Format)
Tests full recomputation vs incremental updates when adding 1, 2, or 3 edges on GPU.
Uses PyTorch sparse tensors with native COO format.
"""
import torch
import numpy as np
import time
...
def coo_to_pytorch_sparse(coo_matrix, device):
    """Convert scipy COO to PyTorch COO sparse tensor."""
...
def add_edges_full_recomputation_gpu(sparse_tensor, new_edges, device, runs=3):
    """
    Add edges by rebuilding entire sparse COO tensor from scratch.
    
    Args:
        sparse_tensor: Original sparse COO tensor
...
def add_edges_incremental_gpu(sparse_tensor, new_edges, device, runs=3):
    """
    Add edges using incremental update (COO→dense→COO conversion).
    
    Note: PyTorch doesn't support in-place COO updates, so we convert to dense.
...
def save_results(results, nodes, sparsity, num_edges):
    ...
    base_filename = f'dynamic_graph_gpu_coo_{nodes}vertices_{sparsity_str}pct_{num_edges}edges'
    ...
    with open(f'results/{base_filename}_results.txt', 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("GPU DYNAMIC GRAPH BENCHMARK (COO Format): Incremental Edge Addition\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='GPU Dynamic Graph Benchmark (COO)')
    ...
    print("=" * 80)
    print("GPU DYNAMIC GRAPH BENCHMARK (COO Format): Incremental Edge Addition")
    ...
    print("Generating base graph (COO format)...")
    base_graph_coo, base_edges, actual_sparsity = generate_graph(args.vertices, args.sparsity)
    print(f"  Base edges: {base_edges:,}")
    print(f"  Actual sparsity: {actual_sparsity:.4f}%")
    print()
    
    print("Transferring to GPU (COO format)...")
    base_graph_gpu = coo_to_pytorch_sparse(base_graph_coo, device)
    print()
    ...
    print(f"  - results/dynamic_graph_gpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.json")
    print(f"  - results/dynamic_graph_gpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.txt")
    print(f"  - results/dynamic_graph_gpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.csv")
    print("=" * 80)
