"""
CPU Dynamic Graph Benchmark: Incremental Edge Addition (COO Format)
...
def add_edges_full_recomputation(base_matrix, new_edges, runs=3):
    """
    Add edges by rebuilding entire COO matrix from scratch.
    
    Args:
        base_matrix: Original COO matrix
    ...
    for _ in range(runs):
        ...
        rows = list(base_matrix.row)
        cols = list(base_matrix.col)
        vals = list(base_matrix.data)
        ...
        new_matrix = sp.coo_matrix((vals, (rows, cols)), shape=base_matrix.shape)
...
def add_edges_incremental(base_matrix, new_edges, runs=3):
    """
    Add edges using incremental update (COO→LIL→COO conversion).
    
    Note: scipy doesn't support efficient COO in-place updates, so we use LIL as intermediate.
...
def save_results(results, nodes, sparsity, num_edges):
    ...
    base_filename = f'dynamic_graph_cpu_coo_{nodes}vertices_{sparsity_str}pct_{num_edges}edges'
    ...
    with open(f'results/{base_filename}_results.txt', 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("CPU DYNAMIC GRAPH BENCHMARK (COO Format): Incremental Edge Addition\n")
        f.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='CPU Dynamic Graph Benchmark (COO)')
    ...
    print("=" * 80)
    print("CPU DYNAMIC GRAPH BENCHMARK (COO Format): Incremental Edge Addition")
    ...
    print(f"  - results/dynamic_graph_cpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.json")
    print(f"  - results/dynamic_graph_cpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.txt")
    print(f"  - results/dynamic_graph_cpu_coo_{args.vertices}vertices_{sparsity_str}pct_{args.num_edges}edges_results.csv")
    print("=" * 80)
