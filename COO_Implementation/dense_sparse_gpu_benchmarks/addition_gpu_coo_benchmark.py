"""
GPU Matrix Addition Benchmark: COO + COO Sparse vs Dense
Compares sparse GPU (PyTorch COO) vs dense GPU addition performance using true COO format.
"""
...
def generate_sparse_coo_matrices(size, sparsity_percent, seed_a=42, seed_b=123):
    """
    Generate two sparse matrices in COO format for addition.
...
def benchmark_gpu_sparse(A_sparse_gpu, B_sparse_gpu, runs=3):
    """
    Benchmark GPU sparse matrix addition (COO + COO).
...
def save_results(results, output_dir, size, sparsity):
    ...
    base_name = f"addition_gpu_coo_{size}x{size}_{sparsity_str}pct_results"
    ...
    with open(txt_path, 'w') as f:
        f.write("=" * 70 + "\n")
        f.write("GPU ADDITION BENCHMARK RESULTS (COO Format)\n")
        f.write("=" * 70 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(description='GPU Sparse Addition Benchmark (COO Format)')
    ...
    print("=" * 70)
    print("GPU ADDITION BENCHMARK: COO Sparse vs Dense")
    print("=" * 70)
    print(f"\nGPU Device: {gpu_name}")
