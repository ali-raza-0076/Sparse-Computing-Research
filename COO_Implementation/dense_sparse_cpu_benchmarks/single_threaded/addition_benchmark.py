...
from sparse_addition_coo import sparse_add_coo
...
def benchmark_sparse_addition(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape, num_runs=3):
    """
    Benchmark sparse COO addition (single-threaded).
    ...
    """
    _ = sparse_add_coo(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape)
    
    times = []
    for _ in tqdm(range(num_runs), desc="  Sparse COO (single-threaded)", leave=False):
        start = time.perf_counter()
        result = sparse_add_coo(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape)
...
def run_comparison(size, sparsity_percent, num_runs=3):
    ...
    print(f"\nBenchmarking SPARSE (COO, single-threaded)...")
    sparse_time, sparse_std, C_sparse = benchmark_sparse_addition(
        rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, (size, size), num_runs
    )
...
def main():
    ...
    print(f"Algorithm: COO (single-threaded)")
    ...
    with open(txt_path, "w", encoding='utf-8') as f:
        ...
        f.write("="*70 + "\n\n")
        f.write(f"Algorithm: COO (single-threaded)\n")
    ...
    print(f"  - addition_{matrix_size}x{matrix_size}_{sparsity_str}_results.json")
    print(f"  - addition_{matrix_size}x{matrix_size}_{sparsity_str}_results.txt")
    print(f"  - addition_{matrix_size}x{matrix_size}_{sparsity_str}_results.csv")
