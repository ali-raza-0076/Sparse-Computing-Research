"""
Dense vs Sparse Matrix Multiplication Benchmark (Single-Threaded)

Compares performance of:
- Dense CPU (NumPy matrix multiplication)
- Sparse CPU (COO×COO single-threaded algorithm)
...
Output:
- results/multiplication_1000x1000_results.txt (human-readable table)
- results/multiplication_1000x1000_results.json (structured data)
- results/multiplication_1000x1000_results.csv (spreadsheet format)
"""
import numpy as np
import time
...
def generate_sparse_matrix(size, sparsity_percent, seed):
    ...
    rows = np.random.randint(0, size, size=num_entries, dtype=np.int32)
    cols = np.random.randint(0, size, size=num_entries, dtype=np.int32)
    values = np.random.randint(1, 11, size=num_entries).astype(np.float64)
    
    return rows, cols, values


def benchmark_sparse_multiplication(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape_A, shape_B, num_runs=3):
    """
    ...
    Args:
        ...
        shape_A, shape_B: Matrix dimensions
        num_runs: Number of iterations to average
    
    Returns:
        avg_time, std_time, result: Average time, std deviation, result COO arrays
    """
    _ = sparse_multiply_coo(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape_A, shape_B)
    
    times = []
    for _ in tqdm(range(num_runs), desc="  Sparse COO×COO (single-threaded)", leave=False):
        start = time.perf_counter()
        result = sparse_multiply_coo(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, shape_A, shape_B)
...
def run_comparison(size, sparsity_percent, num_runs=3):
    ...
    sparse_time, sparse_std, C_sparse = benchmark_sparse_multiplication(
        rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, (size, size), (size, size), num_runs
    )
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark sparse vs dense multiplication (single-threaded)')
    ...
    print("="*70)
    print("MULTIPLICATION BENCHMARK: Dense vs Sparse (Single-Threaded)")
    print("="*70)
    ...
    print(f"Algorithm: COO×COO (single-threaded)")
    ...
    json_path = os.path.join(results_dir, f"multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.json")
    with open(json_path, "w") as f:
        json.dump(results, f, indent=2)
    
    txt_path = os.path.join(results_dir, f"multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.txt")
    with open(txt_path, "w", encoding='utf-8') as f:
        f.write("MULTIPLICATION BENCHMARK: Dense vs Sparse (Single-Threaded)\n")
        f.write("="*70 + "\n\n")
        f.write(f"Algorithm: COO×COO (single-threaded)\n")
    ...
    csv_path = os.path.join(results_dir, f"multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.csv")
    ...
    print(f"  - multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.json")
    print(f"  - multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.txt")
    print(f"  - multiplication_{matrix_size}x{matrix_size}_{sparsity_str}_results.csv")
