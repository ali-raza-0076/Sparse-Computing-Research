"""  
Dense vs Sparse Matrix Multiplication Benchmark (TRUE Multiprocessing)

Compares performance of:
- Dense CPU (NumPy matrix multiplication - single-threaded)
- Sparse CPU (COO×COO TRUE multiprocessing algorithm using 16 separate processes)

Test Configuration:
- Matrix Size: Configurable (default: 1,000×1,000)
- Sparsity Levels: 90%, 99%, 99.9%
- Format: COO (Coordinate List)
- Parallelism: 16-core (AMD Ryzen 9 8940HX) - TRUE multiprocessing
...
Output:
- results/multiplication_multiprocess_{size}x{size}_results.txt
- results/multiplication_multiprocess_{size}x{size}_results.json
- results/multiplication_multiprocess_{size}x{size}_results.csv
"""
import numpy as np
import time
...
from sparse_multiplication_multiprocess_coo import sparse_multiply_coo_multiprocess
...
def benchmark_sparse_parallel(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, size, num_cores, num_runs=3):
    """
    Benchmark sparse COO×COO multiplication with TRUE multiprocessing parallelism.
    
    Uses Python multiprocessing - each core runs in separate process with independent
    memory and computation. This is REAL parallelism that fully utilizes all CPU cores.
    ...
    Returns:
        avg_time, std_time, result_nnz
    """
    times = []
    result_nnz = 0
    
    for _ in tqdm(range(num_runs), desc=f"  Sparse COO×COO ({num_cores}-process TRUE parallel)", leave=False):
        start = time.perf_counter()
        result_rows, result_cols, result_vals = sparse_multiply_coo_multiprocess(
    ...
    return np.mean(times), np.std(times), result_nnz


def benchmark_dense_multiplication(A_dense, B_dense, num_cores, num_runs=3):
    """Benchmark dense NumPy matrix multiplication (uses OpenBLAS multicore)."""
    import os
    ...
    times = []
    for _ in tqdm(range(num_runs), desc=f"  Dense NumPy ({num_cores}-core OpenBLAS)", leave=False):
        start = time.perf_counter()
...
def run_comparison(size, sparsity_percent, num_cores, num_runs=3):
    ...
    print(f"\nBenchmarking SPARSE (COO×COO, {num_cores}-core parallel)...")
    sparse_time, sparse_std, result_nnz = \
    ...
    dense_time, dense_std, C_dense = benchmark_dense_multiplication(A_dense, B_dense, num_cores, num_runs)
    ...
    return {
        ...
        "algorithm": f"COO×COO ({num_cores}-core parallel)",
        "num_runs": num_runs
    }
...
def main():
    ...
    print(f"Algorithm: COO×COO ({num_cores}-core parallel)")
    ...
    base_name = f"multiplication_multiprocess_{matrix_size}x{matrix_size}_{sparsity_str}_results"
    ...
    with open(txt_path, "w", encoding='utf-8') as f:
        f.write("MULTIPLICATION BENCHMARK: Dense vs Sparse (TRUE Multiprocessing)\n")
        f.write("="*70 + "\n\n")
        f.write(f"Algorithm: COO×COO ({num_cores}-process TRUE multiprocessing)\n")
