"""
Dense vs Sparse Matrix Addition Benchmark (Multicore Parallel)

Compares performance of:
- Dense CPU (NumPy array addition - single-threaded)
- Sparse CPU (COO parallel addition using 16 cores)
...
Output:
- results/addition_parallel_1000x1000_results.txt
- results/addition_parallel_1000x1000_results.json
- results/addition_parallel_1000x1000_results.csv
"""
import numpy as np
import time
...
from sparse_addition_parallel_coo import sparse_add_coo_parallel
...
def benchmark_sparse_parallel(rows_A, cols_A, vals_A, rows_B, cols_B, vals_B, size, num_cores, num_runs=3):
    """
    Benchmark sparse COO addition with multicore parallelism.
    ...
    for _ in tqdm(range(num_runs), desc=f"  Sparse COO ({num_cores}-core parallel)", leave=False):
        start = time.perf_counter()
        result_rows, result_cols, result_vals = sparse_add_coo_parallel(
...
def benchmark_dense_addition(A_dense, B_dense, num_runs=3):
    """Benchmark dense NumPy array addition (single-threaded baseline)."""
    times = []
...
def run_comparison(size, sparsity_percent, num_cores, num_runs=3):
    ...
    print(f"\nBenchmarking SPARSE (COO, {num_cores}-core parallel)...")
    ...
    return {
        ...
        "algorithm": f"COO ({num_cores}-core parallel)",
        "num_runs": num_runs
    }


def main():
    parser = argparse.ArgumentParser(description='Dense vs Sparse Matrix Addition Benchmark (Parallel)')
    ...
    print("="*70)
    print("ADDITION BENCHMARK: Dense vs Sparse (Multicore Parallel)")
    print("="*70)
    
    available_cores = mp.cpu_count()
    num_cores = args.num_cores if args.num_cores else min(16, available_cores)
    
    print(f"\nAlgorithm: COO (Coordinate List)")
    ...
    print(f"Algorithm: COO ({num_cores}-core parallel)")
    ...
    base_name = f"addition_parallel_{matrix_size}x{matrix_size}_{sparsity_str}_results"
    ...
    with open(txt_path, "w", encoding='utf-8') as f:
        f.write("ADDITION BENCHMARK: Dense vs Sparse (Multicore Parallel)\n")
        f.write("="*70 + "\n\n")
        f.write(f"Algorithm: COO ({num_cores}-core parallel)\n")
